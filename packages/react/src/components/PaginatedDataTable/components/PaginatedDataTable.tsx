...
export interface PaginatedDataTableProps
  ...
  /** Array of row data (Carbon DataTable format) */
  rows: any[];
 ...
 *   {({ rows, headers, getHeaderProps, getRowProps, getPaginationProps, ...props }) => (
 *     <TableContainer>
 *       <Table>
 *         <TableHead>
 *           <TableRow>
...
export const PaginatedDataTable: React.FC<PaginatedDataTableProps> = React.memo(
  ({
    ...
    const shouldUseWorker = forceWorker || initialRows.length > 10000;
    ...
    useEffect(() => {
      ...
      _sortColumn,
      _sortDirection,
      // Note: initialRows, headers, managers intentionally not in deps to avoid re-processing on every render
    ]);
